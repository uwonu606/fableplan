...
1. **플랜 단계 (fable-planner agent)**
   ...
   - 승인이 거부되면: 피드백 전문을 `SendMessage`로 planner에게 전달해 수정된 플랜을 받고, 다시 승인을 요청한다. 메인 스레드가 플랜을 직접 고치지 않는다.
...
- `fable-planner` agent를 찾을 수 없으면(설치 누락) 메인 스레드가 대신 플랜을 작성하지 말고, install.sh 재실행이 필요하다고 사용자에게 알린다.
