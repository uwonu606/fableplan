...
3. [Opus]   opus-builder agent가 승인된 플랜을 그대로 구현
4. [메인]   테스트/빌드 재실행으로 검증, 결과 보고
```
...
- subagent는 사용자에게 직접 질문할 수 없습니다. planner가 결정이 필요한 지점을 만나면 질문 목록을 반환하고, 메인 스레드가 사용자에게 물어 답을 중계합니다. 플랜 거부 피드백도 같은 경로로 planner에게 전달됩니다 — 한 홉씩 늘어나는 대신, planner가 탐색한 컨텍스트는 유지됩니다.
